...
project_version() {
  ...
  # Append Python version if any .py files exist
  if find . -maxdepth 1 -type f -name "*.py" | grep -q .; then
    version+="${version:+ }🐍 $(python --version 2>&1 | awk '{print $2}')"
  fi

  # Append Node.js version if any .js files exist
  if find . -maxdepth 1 -type f -name "*.js" | grep -q .; then
    version+="${version:+ }🟢 $(node --version 2>&1 | sed 's/v//')"
  fi

  # Append Rust version if any .rs files exist
  if find . -maxdepth 1 -type f -name "*.rs" | grep -q .; then
    version+="${version:+ }🦀 $(cargo --version 2>&1 | awk '{print $2}')"
  fi
...
($(project_version))${PR_HBAR}${PR_CYAN}${PR_LRCORNER}${PR_NO_COLOUR}'
